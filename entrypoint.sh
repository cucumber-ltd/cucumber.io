...
# Set USER with the value for whoami if it's not set
test -z "$USER" && \
    echo "no current user env found, setting to whoami" && \
    USER="$(whoami)"
echo $USER
...
# Fix the python that's used to start
echo "fixing python that's used..."
sed -i -e 's|python|python2|' /usr/bin/nginx-amplify-agent.py
...
test -n "${NGINX_API_KEY}" && \
    api_key=${NGINX_API_KEY}
...
if [ -n "${api_key}" -o -n "${amplify_imagename}" ]; then
    ...
    # chown $USER ${agent_conf_file} > /dev/null 2>&1
    chown $USER ${agent_conf_file}

    test -f "${nginx_status_conf}" && \
    chmod 644 ${nginx_status_conf} && \
    # chown $USER ${nginx_status_conf} > /dev/null 2>&1
    chown $USER ${nginx_status_conf} 
fi
